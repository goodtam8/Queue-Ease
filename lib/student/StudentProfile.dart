...
class Student extends StatefulWidget {
  ...
  const Student({super.key, required this.onLogout});
...
void uploadImage(Uint8List imageData) async {
  ...
  builder: (BuildContext context, AsyncSnapshot<Studentper> snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      return CircularProgressIndicator(); // Show a loading indicator while waiting
    } else if (snapshot.hasError) {
      return Text('Error: ${snapshot.error}');
    } else if (snapshot.hasData) {
      Studentper data = snapshot.data!; // You now have your 'personal' data
 return Center(
              child: Column(
                children: [
                  Stack(
